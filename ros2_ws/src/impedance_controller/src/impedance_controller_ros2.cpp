  ...
  impedance_controller_4_(initializeController())
{
  ...
  measured_position_subscription_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
    ...
    [this](const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
      if (msg->data.size() >= 8) {
        ...
        impedance_controller_1_.setCartesianPositionMeasured(
          finger_kinematics::forward(
            joint_position_1, impedance_controller_1_.getLinkLengths()));
        impedance_controller_2_.setCartesianPositionMeasured(
          finger_kinematics::forward(
            joint_position_2, impedance_controller_2_.getLinkLengths()));
        impedance_controller_3_.setCartesianPositionMeasured(
          finger_kinematics::forward(
            joint_position_3, impedance_controller_3_.getLinkLengths()));
        impedance_controller_4_.setCartesianPositionMeasured(
          finger_kinematics::forward(
            joint_position_4, impedance_controller_4_.getLinkLengths()));
      }
    }
  );
  measured_velocity_subscription_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
    ...
    [this](const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
      if (msg->data.size() >= 8) {
        ...
        impedance_controller_1_.setCartesianVelocityMeasured(
          finger_kinematics::jacobian(
            impedance_controller_1_.getJointPositionMeasured(),
            impedance_controller_1_.getLinkLengths()) * joint_velocity_1);
        impedance_controller_2_.setCartesianVelocityMeasured(
          finger_kinematics::jacobian(
            impedance_controller_2_.getJointPositionMeasured(),
            impedance_controller_2_.getLinkLengths()) * joint_velocity_2);
        impedance_controller_3_.setCartesianVelocityMeasured(
          finger_kinematics::jacobian(
            impedance_controller_3_.getJointPositionMeasured(),
            impedance_controller_3_.getLinkLengths()) * joint_velocity_3);
        impedance_controller_4_.setCartesianVelocityMeasured(
          finger_kinematics::jacobian(
            impedance_controller_4_.getJointPositionMeasured(),
            impedance_controller_4_.getLinkLengths()) * joint_velocity_4);
      }
    }
  );
  ...
  desired_position_subscription_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
    "/desired_joint_position_rad", 10,                                                                                              /* TODO: Check type */
    [this](const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
      if (msg->data.size() >= 8) {
        // Parse message content
        ...
        impedance_controller_1_.setCartesianPositionDesired(
          finger_kinematics::forward(
            joint_position_1, impedance_controller_1_.getLinkLengths()));
        impedance_controller_2_.setCartesianPositionDesired(
          finger_kinematics::forward(
            joint_position_2, impedance_controller_2_.getLinkLengths()));
        impedance_controller_3_.setCartesianPositionDesired(
          finger_kinematics::forward(
            joint_position_3, impedance_controller_3_.getLinkLengths()));
        impedance_controller_4_.setCartesianPositionDesired(
          finger_kinematics::forward(
            joint_position_4, impedance_controller_4_.getLinkLengths()));
      }
    }
      ...
      if (msg->data.size() >= 8) {
        ...
        impedance_controller_1_.setCartesianVelocityDesired(
          finger_kinematics::jacobian(
            ...
            impedance_controller_1_.getLinkLengths()) * desired_joint_velocity_1);
        impedance_controller_2_.setCartesianVelocityDesired(
          finger_kinematics::jacobian(
            impedance_controller_2_.getJointPositionDesired(),
            impedance_controller_2_.getLinkLengths()) * desired_joint_velocity_2);
        impedance_controller_3_.setCartesianVelocityDesired(
          finger_kinematics::jacobian(
            impedance_controller_3_.getJointPositionDesired(),
            impedance_controller_3_.getLinkLengths()) * desired_joint_velocity_3);
        impedance_controller_4_.setCartesianVelocityDesired(
          finger_kinematics::jacobian(
            impedance_controller_4_.getJointPositionDesired(),
            impedance_controller_4_.getLinkLengths()) * desired_joint_velocity_4);
      }
    }
  );
}
