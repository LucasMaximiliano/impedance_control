...
Eigen::Vector2d ImpedanceControllerBase::computeImpedanceTorque()
{
  ...
  Eigen::Vector2d acceleration_error = cartesian_acceleration_desired_ -
    cartesian_acceleration_computed_;
  ...
  if (joint_torque_commanded.hasNaN()) {
    std::cerr <<
      "[ImpedanceControllerBase] Warning: Computed joint torque contains NaN values. Setting to zero."
              << std::endl;
    joint_torque_commanded.setZero();
  }
  if (joint_torque_commanded.cwiseAbs().maxCoeff() > max_actuator_torque_) {
    std::cerr <<
      "[ImpedanceControllerBase] Warning: Computed joint torque exceeds maximum actuator torque. Clamping values."
              << std::endl;
