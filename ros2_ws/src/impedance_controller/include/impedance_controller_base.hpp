...
class ImpedanceControllerBase
{
    public:
        ...
        //! \name Setters
        //! \{
        void setVirtualInertiaMatrix(const Eigen::Matrix2d& virtual_inertia_matrix) { virtual_inertia_matrix_ = virtual_inertia_matrix; }
        void setVirtualDampingMatrix(const Eigen::Matrix2d& virtual_damping_matrix) { virtual_damping_matrix_ = virtual_damping_matrix; }
        void setVirtualStiffnessMatrix(const Eigen::Matrix2d& virtual_stiffness_matrix) { virtual_stiffness_matrix_ = virtual_stiffness_matrix; }
        void setForceFeedbackGain(const double force_feedback_gain) { force_feedback_gain_ = force_feedback_gain; }
        void setControlLoopDuration(const int control_loop_duration) { control_loop_duration_ = control_loop_duration; }

        void setLinkMasses(const Eigen::Vector2d& link_mass_vector) { link_mass_vector_ = link_mass_vector; }
        void setLinkLengths(const Eigen::Vector2d& link_length_vector) { link_length_vector_ = link_length_vector; }
        void setDistToLinkCOM(const Eigen::Vector2d& dist_to_link_com_vector) { dist_to_link_com_vector_ = dist_to_link_com_vector; }
        
        void setMaxActuatorTorque(const double max_actuator_torque) { max_actuator_torque_ = max_actuator_torque; }
        ...
        void setJointPositionDesired(const Eigen::Vector2d& position) { joint_position_desired_ = position; }
        void setJointVelocityDesired(const Eigen::Vector2d& velocity) { joint_velocity_desired_ = velocity; }
        void setJointAccelerationDesired(const Eigen::Vector2d& acceleration) { joint_acceleration_desired_ = acceleration; }
        //! \}
