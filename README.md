...
1. Planner tab
    ...
    - Plot of `/command/set_advanced_torque_nm`: Shows the torque commands being published by the impedance controller to the motors (if enabled in the controller tab). To avoid clutter, only the shoulder and elbow joint torques of the first finger are plotted, but similar values are published for all fingers (given that they started from the same configuration).
2. Controller tab
